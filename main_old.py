...
class CustBoxLayout(WindowDragBehavior, BoxLayout):
    def __init__(self, **kwargs):
        ...
        self.ids.search_field.bind(
            ...
            focus=lambda *args: self.__toggle_search_field_color())

    def __toggle_search_field_color(self):
        field = self.ids.search_field

        print(field.focus)

        if field.focus:
            anim = Animation(opacity=1, t='out_expo', d=.2)
            anim.start(field)

        else:
            anim = Animation(opacity=.3, t='out_expo', d=.2)
            anim.start(field)
